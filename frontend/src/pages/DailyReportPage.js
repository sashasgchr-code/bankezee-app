                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-slate-100">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-left font-medium">Login Bank Details</th>
                        <th className="px-3 py-2 text-left font-medium">Reject Reason</th>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {reportData.leads.map((lead) => {
                        // Login Bank Details - Bank name, amount, ROI where login=Yes
                        const loginBankDetails = lead.login_bank_details || [];
                        let loginBankText = '-';
                        if (loginBankDetails.length > 0) {
                          loginBankText = loginBankDetails.map(b => 
                            `${b.bank}: ${formatCurrency(b.amount)}${b.roi ? ` @ ${b.roi}%` : ''}`
                          ).join(', ');
                        }
                        
                        // Reject Reason - Comprehensive from bank eligibilities
                        const rejectReasons = lead.reject_reasons || [];
                        let rejectReasonText = '-';
                        if (rejectReasons.length > 0) {
                          rejectReasonText = rejectReasons.map(r => 
                            `${r.bank} (${r.type}): ${r.reason}`
                          ).join(' | ');
                        }
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-3 py-3 max-w-[180px]">
                              <div className="text-xs text-green-700" title={loginBankText}>
                                <span className="line-clamp-2">{loginBankText}</span>
                              </div>
                            </td>
                            <td className="px-3 py-3 max-w-[200px]">
                              <div className="text-xs text-red-600" title={rejectReasonText}>
                                <span className="line-clamp-2">{rejectReasonText}</span>
                              </div>
                            </td>
                            <td className="px-3 py-3">
